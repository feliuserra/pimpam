...
async def test_feed_is_chronological(client):
    ...
    await client.post(
        "/api/v1/posts", headers=bob_h, json={"title": "First", "content": "1"}
    )
    await client.post(
        "/api/v1/posts", headers=bob_h, json={"title": "Second", "content": "2"}
    )
    await client.post(
        "/api/v1/posts", headers=bob_h, json={"title": "Third", "content": "3"}
    )
...
async def test_feed_includes_own_posts(client):
    alice_h = await setup_user(client, "alice")
    await client.post(
        "/api/v1/posts",
        headers=alice_h,
        json={"title": "My own post", "content": "This is mine"},
    )
    r = await client.get("/api/v1/feed", headers=alice_h)
    posts = r.json()
    assert len(posts) == 1
    assert posts[0]["title"] == "My own post"

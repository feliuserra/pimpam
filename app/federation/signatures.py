...
async def verify_inbox_signature(
    ...
    if not key_id:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail="Malformed Signature header")
    ...
    try:
        ...
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail="Could not fetch signing actor")

    public_key_pem = actor_doc.get("publicKey", {}).get("publicKeyPem", "")
    if not public_key_pem:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail="Actor has no public key")

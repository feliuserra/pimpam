...
async def record_consent(
    ...
    db.add(ConsentLog(user_id=user_id, consent_type=consent_type, version=version, ip_hash=ip_hash))
...
async def get_user_data_export(db: AsyncSession, user_id: int) -> dict:
    ...
    posts = (await db.execute(
        select(Post).where(Post.author_id == user_id, Post.is_removed == False)  # noqa: E712
        .order_by(Post.created_at.desc())
    )).scalars().all()

    comments = (await db.execute(
        select(Comment).where(Comment.author_id == user_id, Comment.is_removed == False)  # noqa: E712
    ...
    messages_received = (await db.execute(
        select(Message).where(Message.recipient_id == user_id).order_by(Message.created_at.desc())
    )).scalars().all()
    ...
    consent_rows = (await db.execute(
        select(ConsentLog).where(ConsentLog.user_id == user_id).order_by(ConsentLog.created_at)
    )).scalars().all()
    ...
    return {
        ...
        "posts": [
            {"id": p.id, "title": p.title, "content": p.content, "visibility": p.visibility,
             "community_id": p.community_id, "created_at": _ts(p.created_at)}
            for p in posts
        ],
        "comments": [
            {"id": c.id, "post_id": c.post_id, "content": c.content, "created_at": _ts(c.created_at)}
            for c in comments
        ],
        "messages_sent": [
            {"id": m.id, "recipient_id": m.recipient_id, "ciphertext": m.ciphertext, "created_at": _ts(m.created_at)}
            for m in messages_sent
        ],
        "messages_received": [
            {"id": m.id, "sender_id": m.sender_id, "ciphertext": m.ciphertext, "created_at": _ts(m.created_at)}
        ...
        "consent_log": [
            {"consent_type": cl.consent_type, "version": cl.version, "created_at": _ts(cl.created_at)}
